...
class DecisionTreeModel:
    """
    Decision Tree Regressor model for next-step price prediction.
    ...
    def _paths(self, symbol: Optional[str], interval: Optional[str]) -> Tuple[str, str]:
        ...
        key = f"dt_{symbol or 'sym'}_{interval or 'int'}"
    ...
    def train(self, closes: np.ndarray, symbol: Optional[str] = None, interval: Optional[str] = None, force_retrain: bool = False) -> float:
        if not _HAS_DT:
            raise RuntimeError("Decision Tree (scikit-learn) not available on this system")
        if len(closes) < max(60, self.lags + 10):
            raise ValueError("Not enough data for Decision Tree training")
        ...
        self.model = DecisionTreeRegressor(
            max_depth=20,            # Increased from 15 - deeper trees
            min_samples_split=15,    # Decreased from 20 - more splits allowed
            min_samples_leaf=5,      # Decreased from 10 - more granular
    ...
    def predict(self, closes: np.ndarray, horizon: int) -> Tuple[list, float]:
        if self.model is None or not _HAS_DT:
            raise ValueError("Decision Tree model must be trained before predicting")
